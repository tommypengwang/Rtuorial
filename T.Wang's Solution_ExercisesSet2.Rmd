---
title: "T.Wang’s Solution_ExercisesSet2"
author: "T. Wang"
date: "2017/1/26"
output: 
html_document:
...
*Q1*
At least 6 componets needed to explain at 52.29% of the variance in the data
*Q2*
Highest weights of the first principle component is 0.93 in asolute value: 5 yr T-Notes US & 10yr T-Notes
*Q3*
Comp1: US Government Bonds & Notes
Comp2: Currencies
Comp3: Europen Stock Market Indexes
Comp4: US Stock Market Indexes
Comp5: Europen Bonds
Comp6: Oil & Gas
Comp7: Metal Commodities
Comp8: Food Commodities
Comp9: Precious metal commodities
Comp10: Asian Stock Market Index
*Q4*
#Pre Crisis Data

```{r echo=TRUE}

pre_crisis_data = futures_data[as.Date(rownames(futures_data)) > "2002-10-01" & as.Date(rownames(futures_data)) < 2007-10-01]
```
...
*Q1*
It calculates the moving average returns during period day 10  to day 250 at an interval of 20
*Q2*

```{r echo=TRUE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
New_MAfreq<-seq(10,200,by=5)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,New_MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
...
colnames(signal_used) <- colnames(futures_data)
# Now create the returns of the strategy for each futures time series
New_strategy_by_future <- scrub(shift(signal_used,2)*futures_data) # signal 2 days ago
# finally, this is our futures trend following strategy
New_trading_strategy = apply(New_strategy_by_future,1,sum)
names(New_trading_strategy) <- rownames(futures_data)
```
...
```{r}
iplot.pnl(New_trading_strategy)
```

Here is how this strategy has performed during this period:

```{r}
show_data = round(pnl_matrix(New_trading_strategy),2)
iprint.df(show_data)
```
